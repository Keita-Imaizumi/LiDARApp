...
inline bool StrtodDiyFp(const char *decimals, int dLen, int dExp,
	...
	if (actualExp != dExp) {
		static const DiyFp kPow10[] = { DiyFp(
				RAPIDJSON_UINT64_C2(0xa0000000, 0x00000000), -60),  // 10^1
	...
	const int effectiveSignificandSize = Double::EffectiveSignificandSize(
			64 + v.e);
	...
	const uint64_t precisionBits = (v.f & ((uint64_t(1) << precisionSize) - 1))
			* kUlp;
