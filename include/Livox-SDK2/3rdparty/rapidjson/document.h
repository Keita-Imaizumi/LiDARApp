...
struct IsGenericValueImpl<T, typename Void<typename T::EncodingType>::Type,
		typename Void<typename T::AllocatorType>::Type> : IsBaseOf<
...
public:
	...
	typedef typename GenericMemberIterator<true, Encoding, Allocator>::Iterator ConstMemberIterator; //!< Constant member iterator for iterating in
																									 //!< object.
...
public:
	...
	explicit GenericDocument(Type type, Allocator *allocator = 0,
			...
			GenericValue<Encoding, Allocator>(type), allocator_(allocator), ownAllocator_(
					0), stack_(stackAllocator, stackCapacity), parseResult_() {
		if (!allocator_)
	...
	GenericDocument(Allocator *allocator = 0, size_t stackCapacity =
			...
			allocator_(allocator), ownAllocator_(0), stack_(stackAllocator,
...
public:
  ...
  GenericObject AddMember(ValueType &name, std::basic_string<Ch> &value,
	...
	RAPIDJSON_DISABLEIF_RETURN(
			(internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T>>),
			(ValueType &)) AddMember(ValueType &name, T value,
			AllocatorType &allocator) const {
		value_.AddMember(name, value, allocator);
		return *this;
	}
