...
public:
	...
	printf_arg_formatter(internal::basic_buffer<char_type> &buffer,
			...
			base(back_insert_range<internal::basic_buffer<char_type>>(buffer),
					&spec, ctx.locale()), context_(ctx) {
	}

	template<typename T>
	typename std::enable_if<std::is_integral<T>::value, iterator>::type operator()(
			T value) {
	...
	template<typename T>
	typename std::enable_if<std::is_floating_point<T>::value, iterator>::type operator()(
			T value) {
		return base::operator()(value);
	}
...
public internal::context_base<OutputIt,
		basic_printf_context<OutputIt, Char, ArgFormatter>, Char> {
public:
...
private:
	...
	format_arg get_arg(iterator it,
...
template<typename OutputIt, typename Char, typename AF>
typename basic_printf_context<OutputIt, Char, AF>::format_arg basic_printf_context<
		OutputIt, Char, AF>::get_arg(iterator it, unsigned arg_index) {
	(void) it;
...
void basic_printf_context<OutputIt, Char, AF>::format() {
	...
	while (*it) {
		...
		if (spec.has(HASH_FLAG)
			...
			spec.flags = static_cast<uint_least8_t>(spec.flags
					& (~internal::to_unsigned<int>(HASH_FLAG)));
		if (spec.fill_ == '0') {
...
struct basic_printf_context_t {
	typedef basic_printf_context<std::back_insert_iterator<Buffer>,
			typename Buffer::value_type> type;
};
...
inline std::basic_string<Char> vsprintf(const S &format,
		basic_format_args<
				typename basic_printf_context_t<internal::basic_buffer<Char>>::type> args) {
