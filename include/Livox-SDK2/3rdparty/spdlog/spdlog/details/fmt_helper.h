...
inline spdlog::string_view_t to_string_view(
		const fmt::basic_memory_buffer<char, Buffer_Size> &buf)
SPDLOG_NOEXCEPT
{
...
template<size_t Buffer_Size>
inline void append_string_view(spdlog::string_view_t view,
...
inline unsigned count_digits(T n) {
	...
	return static_cast<unsigned>(fmt::internal::count_digits(
			static_cast<count_type>(n)));
}
...
inline ToDuration time_fraction(const log_clock::time_point &tp) {
	...
	return duration_cast < ToDuration > (duration) - duration_cast < ToDuration
			> (secs);
}
