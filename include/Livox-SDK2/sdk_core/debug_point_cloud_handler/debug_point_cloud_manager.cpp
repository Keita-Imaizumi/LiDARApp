...
void DebugPointCloudManager::AddDevice(const uint32_t handle,
	...
	if (devices_info_.find(handle) == devices_info_.end()) {
		...
		devices_info_.emplace(handle, LidarDeviceInfo { detection_data->sn,
				detection_data->dev_type, ip, detection_data->cmd_port });
	}
}

void DebugPointCloudManager::Handler(uint32_t handle, uint16_t lidar_port,
	...
	if (handlers_.find(handle) == handlers_.end()) {
		...
		if (it != devices_info_.end()) {
			handlers_.emplace(handle,
					std::make_shared<DebugPointCloudHandler>(handle,
							it->second.sn, it->second.dev_type, path_));
